...
class EvidenceControllerTest {
    ...
    fun `should return evidence list view and populate model on success`() {
        ...
        fun `should allow manager to view user evidence list`() {
            ...
            mockMvc.perform(get("/evidence/user/$userId"))
                .andExpect(status().isOk)
                .andExpect(view().name("evidence-list"))
                .andExpect(model().attribute("isManagerView", true))
                .andExpect(model().attribute("targetUser", mockUserWithManager))
                .andExpect(model().attribute("evidenceList", listOf(mockEvidence)))
        }
